...
msa_daily <-
  list.files(path = paste0(filepath, 'sensitivity_analysis/MSA')) %>%
  map_df(~read_delim(
    paste0(filepath, 'sensitivity_analysis/MSA/', .),
    delim = '\001',
...
head(msa_daily)
table(msa_daily$day_week)

# Load MSA data (work vs non-work)
#-----------------------------------------

# Only keep Mon-Fri
msa_hours <- read_delim(paste0(filepath, 'MSA_work_nonwork/20240522_211935_00013_w9qnv_22bd47be-7c29-4997-b2f2-388a2f497a9c.gz'),
                  delim = '\001',
                  col_names = c('msa_name', 'provider_id', 'event_date',
                                'work_hh_devices', 'non_work_hh_devices'),
                  col_types = 'cciii') %>%
  data.frame() %>%
  mutate(date = as.Date(as.character(event_date), format = "%Y%m%d"),
         day_week = wday(date, label = T)) %>%
...
head(msa_hours)
table(msa_hours$day_week)

# Combine MSA data (daily + work vs non-work)
#-----------------------------------------

msa <- rbind(msa_daily, msa_hours)
table(msa$day_week)
...
final_df <-
  ...
  select(-day_week) %>%
  left_join(msa %>% 
              rename(msa_non_work_hh_devices = non_work_hh_devices,
                           msa_work_hh_devices = work_hh_devices) %>%
              select(-day_week), 
            by = c('msa_name', 'provider_id', 'date')) %>%
...
for_imputation <-
  ...
  summarize(work_hh_devices = sum(work_hh_devices, na.rm = T),
            ...
            msa_work_hh_devices = sum(msa_work_hh_devices, na.rm = T),
            msa_non_work_hh_devices = sum(msa_non_work_hh_devices, na.rm = T)) %>%
  ungroup() %>%
  data.frame() %>%
  mutate(normalized_work_hrs = work_hh_devices/msa_work_hh_devices,
         normalized_nonwork_hrs = non_work_hh_devices/msa_non_work_hh_devices)
...
rq_us <-
  ...
  summarize(comm_wrk = sum(work_hh_devices, na.rm = T),
            ...
            msa_non_work_hh_devices = sum(msa_non_work_hh_devices, na.rm = T)) %>%
  ungroup() %>%
  mutate(norm_wrk = comm_wrk/msa_work_hh_devices,
         norm_nonwrk = comm_nonwrk/msa_non_work_hh_devices) %>%
  pivot_wider(
  ...
  arrange(desc(rq_wrk)) %>%
  filter(!comm_district %in% 
           c('San Francisco: NEIGHBORHOOD COMMERCIAL, CLUSTER - 19716',
             'San Francisco: NEIGHBORHOOD COMMERCIAL, CLUSTER - 19692'))

head(rq_us)
rq_us %>% filter(is.na(rq_wrk) | is.na(rq_nonwrk)) # should be none
...
comm_sf2 <- rq_sf %>%
  mutate(
    ...
    levels = c('<60%', '60 - 69%', '70 - 79%', '80 - 89%', '90 - 99%',
    ...
    rate_nonwrk_cat = factor(case_when(
      ...
      rq_nonwrk < .9 ~ '80 - 89%',
      rq_nonwrk < 1 ~ '90 - 99%',
      rq_nonwrk < 1.1 ~ '100 - 109%',
      rq_nonwrk < 1.2 ~ '110 - 119%',
      rq_nonwrk < 1.3 ~ '120 - 129%',
      TRUE ~ '130%+'
    ),
    levels = c('<60%', '60 - 69%', '70 - 79%', '80 - 89%', '90 - 99%',
               '100 - 109%', '110 - 119%', '120 - 129%', '130%+')),   
