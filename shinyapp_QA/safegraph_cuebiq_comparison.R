...
safegraph_arima <- lapply(safegraph_ts %>% select(-date_range_start), auto.arima)

safegraph_arima$Albuquerque
...
cuebiq_arima <- lapply(cuebiq_ts %>% select(-date_range_start), auto.arima)

cuebiq_arima$Albuquerque       
...
for (cities in cuebiq_data_stoppers %>% distinct(city) %>% arrange(city) %>% pull(city)) {
  ...
  SG <- comparisons_df %>%
    dplyr::filter((source == 'safegraph') & (city == cities)) %>%
    arrange(date_range_start) %>%
    pull(normalized_visits_by_total_visits) %>%
    as.vector()
...
ggplotly(downtown_rq %>%
        dplyr::filter((week >= "2020-03-16") & (metric == "downtown")) %>%
...
write.csv(downtown_rq, "git/downtownrecovery/shinyapp/input_data/all_weekly_metrics_cuebiq_update_hll.csv")
