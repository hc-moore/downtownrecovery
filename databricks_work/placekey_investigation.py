...
dbutils.widgets.dropdown('week_end', '2021-06-07', study_weeks)
dbutils.widgets.dropdown('week_start', '2020-06-01', study_weeks)
...
# MAGIC 
# MAGIC $$ Y(y, w, c, g) = \sum_g X(y, w, c, g) $$
# MAGIC 
...
# MAGIC 
# MAGIC $$ R(y, w, c, g) = E(\frac{Y(y, w, c, g)}{Y(y = 2019, w, c, g)}) $$
# MAGIC 
...
# MAGIC %md
# MAGIC # Creating LQ / RQ

# COMMAND ----------

sampled_placekeys_us=  get_table_as_pandas_df("us_zipcode_placekey_202010")
sampled_placekeys_can = get_table_as_pandas_df("postal_code_placekey_can")
all_city_index = get_table_as_pandas_df('all_city_index')

all_city_index['postal_code'] = all_city_index['postal_code'].fillna(0).astype(int)
sampled_placekeys_us['postal_code'] = sampled_placekeys_us['postal_code'].fillna(0).astype(int)
#sampled_placekeys_us_202206['postal_code'] = sampled_placekeys_us_202206['postal_code'].fillna(0).astype(int)
sampled_placekeys_can['postal_code'] = sampled_placekeys_can['postal_code'].fillna(0).astype(int)

# COMMAND ----------

sampled_placekeys_can.head()

# COMMAND ----------

sampled_placekeys_us.head()

# COMMAND ----------

sampled_placekeys_us = sampled_placekeys_us[['postal_code', 'placekey']].merge(all_city_index, on = 'postal_code', how = 'inner')
...
all_placekeys = pd.concat([sampled_placekeys_can, sampled_placekeys_us])

# COMMAND ----------

# MAGIC %md
# MAGIC # Sampling placekeys
# MAGIC 
# MAGIC Canadian placekeys were acquired by subsetting Safegraph's Places dataset. The relevant fields of Places are `placekey` and `postal_code`. Only placekeys with the first 3 characters of `postal_code` (FSA) that matched the FSA to CMA boundary shapefile from statcan were included in batch requests of 20 to Safegraph's API. 
# MAGIC 
# MAGIC Placekeys were pulled in identical order for every week requested. 123,655 total unique placekeys are ultimately present in the final dataframe.
# MAGIC 
# MAGIC (Describe US placekey collection / how it was determined / performed here)
# MAGIC 
# MAGIC # Variations within sampling methods and data collection
# MAGIC 
# MAGIC To both validate Safegraph's data and our own consistency in collection / reporting, here is a post mortem of it all. Licensing whatever means we can't share it individually, so any and all reported numbers, figures (both text and visual) are of Safegraph Patterns data aggregated to the city and postal code (ZIP in the US, DAUID in Canada) level. No aggregated data from Safegraph is ever displayed. Only the population parameter estimates of `raw_visit_counts` by state using the relationship between `raw_visit_counts` and `normalized_visits_by_total_visits`. Dimensions, record counts, and filtering methods are displayed for transparency and validation of *this* analysis. 
...
all_sampled_records['week_num'] = pd.to_datetime(all_sampled_records['date_range_start'].values).week.astype(int)
all_sampled_records['year'] = pd.to_datetime(all_sampled_records['date_range_start'].values).year
all_sampled_records.loc[all_sampled_records['year'] == 2020, 'week_num'] = all_sampled_records.loc[all_sampled_records['year'] == 2020, 'week_num'] - 1
...
def format_ts_numpy():
    rq_df = cluster_cities.copy()
    rq_df = cluster_cities[['city', 'normalized_visits_by_total_visits', 'week']]
    rq_df = rq_df.dropna()
    rq_df = rq_df.pivot_table(index='city', columns='week', values='normalized_visits_by_total_visits', aggfunc='max').T.reset_index().drop(columns=['week'])
    #cols_to_drop = ['Dallas', 'Orlando', 'Oklahoma City', 'Mississauga', 'Hamilton'] + ['Hamilton', 'Montreal', 'Quebec']
   # rq_df = rq_df[]
...
metrics = metrics.drop_duplicates(subset = ['city', 'week', 'metric']).drop(columns = ['raw_visit_counts', 'normalized_visits_by_state_scaling'])
metrics['week_num'] = pd.to_datetime(metrics['week'].values).week.astype(int)
metrics['year'] = pd.to_datetime(metrics['week'].values).year
metrics.loc[metrics['year'] == 2020, 'week_num'] = metrics.loc[metrics['year'] == 2020, 'week_num'] - 1
...
metrics = metrics.replace('Québec','Quebec')
metrics = metrics.replace('Montréal','Montreal')

metrics_wc =  metrics[metrics['metric'] == dbutils.widgets.get('metric')].merge(metric_clusters, on = 'city', how = 'inner')
dbutils.widgets.dropdown('descriptions', metrics_wc['description'].unique()[0], metrics_wc['description'].unique())

cluster_cities = metrics_wc[(metrics_wc['description'] == dbutils.widgets.get('descriptions')) & (~metrics_wc['city'].isin(['Dallas', 'Orlando', 'Oklahoma City', 'Mississauga', 'Hamilton'] + ['Hamilton', 'Montreal', 'Quebec']))]

from scipy.spatial.distance import euclidean
from fastdtw import fastdtw

X_df, ts_format_dt_rq = format_ts_numpy()
ts_format_period_rq = ts_format_dt_rq[first_week_index:last_week_index] # period 2
X_df_rq = X_df.iloc[first_week_index:last_week_index, :] # period 2
shape = ts_format_period_rq.shape
X_df_rq_scaled = TimeSeriesScalerMeanVariance().fit_transform(X_df_rq.to_numpy()).reshape(shape[0],shape[1])
#ts_format_dt_rq_scaled = to_time_series_dataset(ts_format_dt_rq_scaled)

cities_in_cluster = cluster_cities['city'].unique()
end_a = len(cities_in_cluster) // 2

fig, ax = plt.subplots(nrows = len(cities_in_cluster[:end_a]),
                       ncols = len(cities_in_cluster[end_a:]),
                       figsize=(end_a * 6, end_a * 6),
                       sharey = False,
                       sharex = True,
                       constrained_layout = True)
fig.suptitle(dbutils.widgets.get('descriptions') + ' - Period ' + dbutils.widgets.get('period') + ' ' + dbutils.widgets.get('metric') + ' recovery cluster DTW approximation')

for i, city1 in  enumerate(cities_in_cluster[:end_a]):
    for j, city2 in enumerate(cities_in_cluster[end_a:]):
        if city1 != city2:
            
            X1 = pd.DataFrame(X_df_rq_scaled, columns = X_df_rq.columns)[city1]
            X2 = pd.DataFrame(X_df_rq_scaled, columns = X_df_rq.columns)[city2]
            ...
            for [map_x, map_y] in warp_path:
                ax[i,j].plot([map_x, map_y], [x[map_x], y[map_y]], '--k', linewidth=4)
            ax[i,j].plot(x, color='blue', marker='o', markersize=10, linewidth=5, label = city1)
            ax[i,j].plot(y, color='red', marker='o', markersize=10, linewidth=5, label = city2)
            ax[i,j].legend();
plt.show();
...
# MAGIC %md
# MAGIC # Cluster time series plots
...
cluster_cities = metrics_wc[~metrics_wc['city'].isin(['Dallas', 'Orlando', 'Oklahoma City', 'Mississauga', 'Hamilton'] + ['Hamilton', 'Montreal', 'Quebec'])]
...
comparisons = [[['Los Angeles', 'Cincinnati'], ['Colorado Springs', 'Wichita']], [['Halifax', 'San Antonio'], ['San Diego', 'Albuquerque']]]
end_a = len(cities_in_cluster)
#comparison_1 = 
#comparison_1 = np.random.choice(cities, 6, replace = False)
my_labels = {"Los Angeles" : "Los Angeles", "Cincinnati" : "Cincinnati", 'Colorado Springs':'Colorado Springs', 'Wichita':'Wichita', 'Halifax':'Halifax', 'San Antonio':'San Antonio', 'San Diego':'San Diego', 'Albuquerque':'Albuquerque'}

#comparison_2 = ['Honolulu', 'Memphis']
fig, ax = plt.subplots(nrows = 2,
                       ...
                       sharex = True,
                       tight_layout = True)
#fig.suptitle('Period ' + dbutils.widgets.get('period') + ' ' + dbutils.widgets.get('metric') + ' recovery cluster DTW approximation')
#fig.suptitle(dbutils.widgets.get('metric') + ' recovery cluster DTW approximation')
for i, cities1 in enumerate(comparisons[0]):
    X1 = pd.DataFrame(X_df_rq_scaled, columns = X_df_rq.columns)[cities1[0]]
    X2 = pd.DataFrame(X_df_rq_scaled, columns = X_df_rq.columns)[cities1[1]]
    dtw_distance, warp_path = fastdtw(X1, X2, dist=euclidean)
    x = X1.to_numpy()
    y = X2.to_numpy()
    for [map_x, map_y] in warp_path:
        ax[0, i].plot([map_x, map_y], [x[map_x], y[map_y]], '--k', linewidth=4)
    ax[0,i].plot(x, color='blue', marker='o', markersize=10, linewidth=5, label = my_labels[cities1[0]])
    ax[0,i].plot(y, color='red', marker='o', markersize=10, linewidth=5, label = my_labels[cities1[1]])
    if ~i:
        props = dict(boxstyle='round', facecolor='green', alpha=0.5)
        textstr = 'Same Cluster'
        ax[0,i].text(25, 0.5, textstr,  fontsize=34, verticalalignment='top', bbox=props)
    else:
        props = dict(boxstyle='round', facecolor='green', alpha=0.5)
        textstr = 'Same Cluster'
        ax[0,i].text(25, .65, textstr,  fontsize=34, verticalalignment='top', bbox=props)
    my_labels[cities1[0]] = "_nolegend_"
    my_labels[cities1[1]] = "_nolegend_"
    ax[0,i].tick_params(axis='both', which='major', labelsize=32)
    ax[0,i].legend(numpoints = 1, prop={'size': 32});
#plt.show();
    
for j, cities2 in enumerate(comparisons[1]):
    X1 = pd.DataFrame(X_df_rq_scaled, columns = X_df_rq.columns)[cities2[0]]
    X2 = pd.DataFrame(X_df_rq_scaled, columns = X_df_rq.columns)[cities2[1]]
    dtw_distance, warp_path = fastdtw(X1, X2, dist=euclidean)
    x = X1.to_numpy()
    y = X2.to_numpy()
    for [map_x, map_y] in warp_path:
        ax[1, j].plot([map_x, map_y], [x[map_x], y[map_y]], '--k', linewidth=4)
    ax[1,j].plot(x, color='blue', marker='o', markersize=10, linewidth=5, label = my_labels[cities2[0]])
    ax[1,j].plot(y, color='red', marker='o', markersize=10, linewidth=5, label = my_labels[cities2[1]])
    if ~j:
        props = dict(boxstyle='round', facecolor='red', alpha=0.5)
        textstr = 'Different Cluster'
        ax[1,j].text(32, 0.5, textstr,  fontsize=34, verticalalignment='top', bbox=props)
    else:
        props = dict(boxstyle='round', facecolor='red', alpha=0.5)
        textstr = 'Different Cluster'
        ax[1,j].text(32, 0.15, textstr,  fontsize=34, verticalalignment='top', bbox=props)
    my_labels[cities2[0]] = "_nolegend_"
    my_labels[cities2[1]] = "no_legend_"
    ax[1,j].tick_params(axis='both', which='major', labelsize=32)
    ax[1,j].legend(numpoints = 1, prop={'size': 32});
plt.show();
...
metrics_wc[metrics_wc['description'] == dbutils.widgets.get('descriptions')]
...
#weekly_nvt_city =
#weekly_nvt_city = weekly_nvt_city#.pivot_table(columns = 'year', values = 'normalized_visits_by_total_visits', index = ['week_num', 'city']).reset_index().rename(columns = {2019 : 'year_2019', 2020: 'year_2020', 2021: 'year_2021',  2022: 'year_2022'})
#weekly_nvt_city
...
cluster_cities = metrics_wc[metrics_wc['description'] == dbutils.widgets.get('descriptions')]

city_1 = cluster_cities[cluster_cities['city'] == cluster_cities['city'].unique()[0]]
city_2 = cluster_cities[cluster_cities['city'] == cluster_cities['city'].unique()[1]]
...
dtw_distance, warp_path = fastdtw(city_1.sort_values(by = 'week')['normalized_visits_by_total_visits'].dropna(), city_2.sort_values(by = 'week')['normalized_visits_by_total_visits'].dropna(), dist=euclidean)
...
x = city_1.sort_values(by = 'week')['normalized_visits_by_total_visits'].dropna().to_numpy()
y = city_2.sort_values(by = 'week')['normalized_visits_by_total_visits'].dropna().to_numpy()


for [map_x, map_y] in warp_path:
    ax.plot([map_x, map_y], [x[map_x], y[map_y]], '--k', linewidth=4)
    
ax.plot(x, color='blue', marker='o', markersize=10, linewidth=5, label = city_1['city'].unique())
ax.plot(y, color='red', marker='o', markersize=10, linewidth=5, label = city_2['city'].unique())
#g = sns.relplot(data = weekly_nvt_city[weekly_nvt_city['city'] == dbutils.widgets.get('city')], x = 'week_num', y = 'value', hue = 'year', units = 'year', kind="line", palette="crest", estimator = None, linewidth=4, zorder=5, height=10, aspect=1.5, facet_kws = {'sharex':False})
...
#    sns.lineplot(
#        data=weekly_nvt_plot, x="week_num", y="value", units="year",
...
#g.set_titles("")
#g.set_axis_labels("", "Normalized visits by total visits")
#g.tight_layout()
...
rq_dwtn_clusters['metric'] = 'downtown'
rq_city_clusters = get_table_as_pandas_df("rq_city_clusters_0822") 
rq_city_clusters['metric'] = 'metro'
all_clusters = pd.concat([lq_clusters, rq_dwtn_clusters, rq_city_clusters])
all_clusters

# COMMAND ----------

X = all_sampled_records.merge(metrics_w_clusters, right_on = ['city', 'week'], left_on = ['city', 'date_range_start'], how = 'inner')
X
