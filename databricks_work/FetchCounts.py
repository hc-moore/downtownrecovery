...
cities_blacklist = ["Virginia Beach", "Mesa", "Arlington", "Aurora", "Long Beach"]
cities_whitelist = ["New York"]

city_index_df = city_index_full_df[city_index_full_df["city"].isin(cities_whitelist)]
...
async def fetch_key_all(keys_df, start, end):
    for city in keys_df["city"].drop_duplicates():
        ...
        append_table(result, "safegraph_patterns_count")
        print("SAVED DATA")
    

date = datetime.date.fromisocalendar(2020, 10, 1)
counts_df = await fetch_key_all(
    key_city_df, date, date + datetime.timedelta(weeks=1)
)
counts_df
...
# counts=get_table_as_pandas_df("safegraph_patterns_count")
# counts

# COMMAND ----------

# merged = counts.merge(key_city_df, on="placekey")
# merged

# COMMAND ----------

# by_zip = merged.groupby(["city", "postal_code", "date_range_start"], as_index=False).aggregate("sum")
# by_zip

# COMMAND ----------

# create_replace_table(by_zip, "safegraph_patterns_count_agg_postal")

# COMMAND ----------

# table.groupby(["postal_code", "date_range_start"], as_index=False).aggregate("sum")
