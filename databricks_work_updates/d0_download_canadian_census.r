...
CSD_regions <- list_census_regions('CA16') %>% 
  filter(level == "CSD", name %in% c("Vancouver","Toronto", "Québec", "Montréal",
                                     "Ottawa", "Edmonton", "London", "Halifax",
                                     "Mississauga", "Calgary", "Winnipeg")) %>%
  pull(region)
...
all16_census_vecs %>%
  filter(str_detect(details, "Minority\\W+Origin") &
           (type == "Total")) %>%
  select(vector, label, parent_vector, details) %>%
  pull(vector, label)

visible_minority_vecs <- all16_census_vecs %>%
  filter(str_detect(details, "isible") &
           (type == "Total")) %>%
  select(vector, label, parent_vector, details)

visible_minority_vecs %>% pull(vector, label) %>% print(n = Inf, na.print = "")

national_origin_vecs <- all16_census_vecs %>%
  filter(str_detect(details, "Minority\\W+Origin") &
           (type == "Total") & str_detect(label, "[Oo]rigin")) %>%
  select(parent_vector, vector, label)

national_origin_vecs %>% print(n = Inf)

# only in R
sort(setdiff(census_vecs, python_colnames))

# only from python
sort(setdiff(python_colnames, census_vecs))

query_census_vecs <- unique(c(python_colnames, national_origin_vecs %>% pull(vector), visible_minority_vecs %>% pull(vector)))


# run this once to make a dataframe
census_data <- get_census(dataset = "CA16", regions = list(CSD = CSD_regions),
                          vectors = query_census_vecs, level = "CSD")

census_data %>% glimpse()

# run it on the rest and bind the dataframes together by row
for (CSDs in CSD_UIDs[-1]) {
  
  da_data <- get_census(dataset = "CA16", regions = list(CSD = CSDs),
                        vectors = query_census_vecs, level = "DA")
  census_data <- rbind(census_data, da_data)
...
write.csv(census_data, "~/data/downtownrecovery/census_data/canadian_census_data_CSD.csv")
