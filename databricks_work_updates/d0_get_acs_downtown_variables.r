...
# load downtown information
# these are the placeids:

# COMMAND ----------

#Import Downtown Zipcodes
us_city_index <- read.csv('~/data/downtownrecovery/geographies/city_index_0119.csv')

us_city_index %>% glimpse()

us_city_index <- us_city_index %>%
                    select(-X, -X_c0, -geo, city = X0, state = X1, STATEFP = X2, PLACEFP = X3, zipcodes)

us_city_index %>% glimpse()

us_downtowns <- read.csv('~/data/downtownrecovery/geographies/us_downtowns.csv', header = FALSE)
us_downtowns %>% glimpse()

us_downtowns <- us_downtowns %>% select(city = V1, state = V2, downtown_zipcodes = V3, method = V4)
us_downtowns %>% glimpse()

us_city_index <- us_city_index %>%
                    filter(!(city %in% c("Mesa", "Long Beach", "Virginia Beach", "Arlington", "Aurora")))

us_city_index_long <- us_city_index %>%
                        separate_longer_delim(zipcodes, delim = "',") %>%
                        mutate(zipcodes = str_pad(str_remove_all(zipcodes, "\\D"), side = "left", 5, "0")
                        )

us_downtowns_long <- us_downtowns %>%
                        separate_longer_delim(downtown_zipcodes, delim = "',") %>%
                        mutate(downtown_zipcodes = str_pad(str_remove_all(downtown_zipcodes, "\\D"), side = "left", 5, "0")
                        )
                        

us_city_index_long %>% glimpse()


us_city_index_long%>%
    left_join(us_downtowns_long %>% select(city, state, downtown_zipcodes), by = c("state", "city", "zipcodes" = "downtown_zipcodes")) %>%
    glimpse()
...
mutate_vars <- function(df){
  df_new <- df %>%
    mutate(
               ...
               pct_commute_others_city=100 - pct_commute_auto_city - pct_commute_public_transit_city - pct_commute_bicycle_city - pct_commute_walk_city,
