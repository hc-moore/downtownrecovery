...
can_city_index =  pd.read_csv('~/data/downtownrecovery/geographies/can_city_index_da_0406.csv')
can_census_data =  pd.read_csv('~/data/downtownrecovery/census_data/canadian_census_data_1015.csv')
can_cbp_data =  pd.read_csv('~/data/downtownrecovery/business_data/cpb2016da__2011__csv.csv')
...
can_census_data.columns = can_census_data.columns.str.replace(" |\\(|\\)|,", "_")
...
np.mean((can_census_city["v_CA16_4900:_Median_monthly_shelter_costs_for_rented_dwellings__$_"].astype(float)))*(1.0143*1.016*1.0227*0.7338),
                    ...
                    #city_area  
                    float(can_city_sum["Area__sq_km_"])])#*0.386102 (as above)
...
# COMMAND ----------
# should be strings
can_cbp_data["GEO"] = can_cbp_data["GEO"].astype(str)

# COMMAND ----------

can_cbp_data.iloc[:,-20:].to_numpy().sum()
...
can_city_index.iloc[:,1] = can_city_index.iloc[:,1].str.replace("é", "e")

# COMMAND ----------

can_city_index

can_city_index.to_csv('~/data/downtownrecovery/curated_data/can_city_index_with_features_20230705.csv')
