...
counts_df_covid = get_table_as_pandas_df("new_york_device_count")
counts_df_covid
...
counts_df_covid_zipcode = counts_df_covid
...
a=counts_df_covid_zipcode[counts_df_covid_zipcode["date_range_start"].str.contains("2021-09")]
a
...
formatted_df = counts_df.drop(
    columns=["raw_visit_counts", "normalized_visits_by_total_visits", "city"]
...
city_list_df[city_list_df["city"] == "New York"]
...
#     if week < 9:
    ...
    old_metric = existing[existing["city"] == city]["rec_2021." + str(week)].values[0]
